...
- 지정된 영문 타입을 제외하고 커밋 제목과 본문은 한글로만 작성한다.
- 제목은 `타입:제목` 형식으로 작성한다. 콜론 뒤에는 공백을 넣지 않는다.
- 제목은 변경 목적이 드러나도록 간결하고 구체적으로 작성한다.
- 타입은 아래 일곱 가지 중 변경 목적에 맞는 하나만 사용한다.

| 타입 | 용도 |
| --- | --- |
| feature | 새로운 기능 추가 |
| test | 테스트 추가 또는 수정 |
| style | 동작 변경 없는 코드 서식·스타일 수정 |
| docs | 문서 추가 또는 수정 |
| refactor | 기존 동작을 유지하는 코드 구조 개선 |
| fix | 오류 및 잘못된 동작 수정 |
| config | 환경·빌드·실행 설정 변경 |

제목 다음에는 빈 줄을 두고, 본문은 변경 사항을 요약한 2~3줄로 작성한다.
각 줄에는 주요 변경 내용과 그 목적을 적어, 커밋 메시지만으로도 변경 이력을 파악할 수 있게 한다.

예시:

```text
feature:콘텐츠 공개 기간 설정 추가

관리자가 콘텐츠의 공개 시작일과 종료일을 설정할 수 있도록 추가한다.
공개 기간이 지난 콘텐츠는 목록과 새 재생 요청에서 제외한다.
```

## 커밋 단위

- 하나의 커밋에는 하나의 명확한 목적에 필요한 변경만 포함한다.
- 다른 사람이 리뷰하기 쉽고 변경 이력을 추적할 수 있도록 변경 코드 줄 수를 작게 유지한다.
- 독립적으로 설명하고 검증할 수 있는 변경은 별도 커밋으로 나눈다.
- 기능 변경과 무관한 리팩터링·서식 정리·설정 변경을 한 커밋에 섞지 않는다.
- 줄 수를 줄이기 위해 동작에 함께 필요한 코드를 억지로 분리하지 않는다. 각 커밋은 빌드와 관련 검증이 가능한 일관된 상태를 유지한다.
- 커밋 전에 포함할 변경의 차이를 확인하고, 해당 목적과 관계없는 변경을 제외한다.
