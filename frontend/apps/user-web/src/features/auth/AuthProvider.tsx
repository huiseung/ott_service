...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { let active = true; userApi.me().then(value => { if (active) setUser(value); }).catch(() => userApi.refresh().then(value => { if (active) setUser(value); }).catch(() => {})).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, []);
